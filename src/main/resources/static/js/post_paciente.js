window.addEventListener("load", function () {
  ...
  formulario.addEventListener("submit", function (event) {
    ...
    //invocamos utilizando la función fetch la API pacientes con el método POST que guardará el paciente que enviaremos en formato JSON
    const url = "/pacientes";
    const settings = {
      method: "POST",
      headers: {
    ...
    fetch(url, settings)
      ...
      .catch((error) => {
        //Si hay algun error se muestra un mensaje diciendo que el paciente no se pudo guardar y se intente nuevamente
        let errorAlert =
